...
def display_menu():
    ...
    print('--------------------------------------------------------')
    print('1 - Display All')
    print('2 - Create a New Major')
    print('3 - Read a Major')
    print('4 - Update a Major')
    print('5 - Delete a Major')
    print('6 - EXIT')
...
def display_all(cur):
    # Get all the majors.
    cur.execute('SELECT MajorName FROM Majors ORDER BY MajorName')
...
def create_row(cur):
    ...
    print('New major added.\n')

# Read a row. (Search by major Name.)
def read_row(cur):
    # Get the name to search for.
    name = input('Major to search for: ')

    # Insert the % symbols before and after the MajorName.
    name = '%' + name + '%'

    # Search for a row containing the MajorName.
    cur.execute('''SELECT MajorName FROM MAjors
                   WHERE MajorName LIKE ?''', (name,))
...
def update_row(cur):
    # Get the existing name of the major.
    name = input('Major to search for: ')
    ...
    if len(results) > 0:
        ...
        id = int(input('Enter the ID of the major you wish to update: '))
        if contains_id(results, id):
            new_name = input('Enter the new major name: ')
            cur.execute('''UPDATE Majors SET MajorName = ?
                           WHERE MajorID == ?''', (new_name, id))
            print('Entry updated.\n')
        else:
...
def delete_row(cur):
    ...
    # Insert the % symbols before and after the Major Name.
    name = '%' + name + '%'
    ...
    if len(results) > 0:
        ...
        for row in results:
            print(f'{row[0]:<3} {row[1]:20}')
        id = int(input('Enter the ID of the major you wish to delete: '))
        if contains_id(results, id):
